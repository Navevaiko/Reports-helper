...
window.exportData.addEventListener('submit', async event => {
    ...
    if (Object.keys(context).length == 1) {
        ...
        dataCard = cards.data.map(async card => {
            ...
            const resquest = await axios.get(`https://api.trello.com/1/cards/${id}/pluginData?key=${secret}&token=${token}`);
        ...
        // dataCard =  cards.data.map( card => {
        //     let idCard = card.id;

        //     let report = axios.get(`https://api.trello.com/1/cards/${idCard}/pluginData?key=${secret}&token=${token}`);
